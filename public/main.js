...
socket.on('join_room_response', (payload) => {
  if ((typeof payload == 'undefined') || (payload === null)) {
    console.log('Server did not send the payload');
    return;
  }
  if (payload.result === 'fail') {
    console.log(payload.message);
    ...
    for (let column = 0; column < 8; column++) {
      ...
      if (old_board[row][column] !== board[row][column]) {
        ...
        if ((old_board[row][column] === '?') && (board[row][column] === ' ')) {
          ...
          graphic = "blank_to_white_token.gif";
          altTag = "white token ";
        }

        else if ((old_board[row][column] === '?') && (board[row][column] === 'b')) {
          graphic = "blank_to_black_token.gif";
          altTag = "black token ";
        }
          ...
          graphic = "white_token_to_white_space.gif";
          altTag = "empty space";
        }

        else if ((old_board[row][column] === 'b') && (board[row][column] === ' ')) {
          graphic = "black_token_to_white_space.gif";
          altTag = "empty space";
        }

        else if ((old_board[row][column] === 'w') && (board[row][column] === 'b')) {
          graphic = "white_to_black_token.gif";
          altTag = "black token";
        }

        else if ((old_board[row][column] === 'b') && (board[row][column] === 'w')) {
          graphic = "black_to_white_token.gif";
          altTag = "white token";
        }
