...
function makeInvitedButton(socket_id) {
  let newHTML = "<button type='button' class='btn custom-button'>Invited</button>";
  let newNode = $(newHTML);
  newNode.click(() => {
      let payload = {
          requested_user: socket_id
...
function makeStartGameButton() {
  let newHTML = "<button type='button' class='btn btn-success'>Starting Game</button>";
  let newNode = $(newHTML);
  return newNode;
}
...
socket.on('join_room_response', (payload) => {
  ...
  /* If we are being notified our selfves then ignore the message and return */
...
socket.on('game_update', (payload) => {
  ...
  if (typeof board === 'undefined' || board === null) {
      console.log('Server did not send a valid board to play');
      return;
  }
  ...
  if (my_color === 'white') {
      $("#my_color").html('<img src="assets/images/white-token-splash.png" alt="Player White"><h3 id="my_color" style="background-color: #87a5a8; border: 1px solid white; width: 120px; margin-left: auto; margin-right: auto; margin-top: -10px; margin-bottom: 10px; font-size: 15px; border-radius: 3px;">Team White</h3>');
  }
  else if (my_color === 'black') {
      $("#my_color").html('<img src="assets/images/black-token-splash.png" alt="Player Black"><h3 id="my_color" style="background-color: #87a5a8; border: 1px solid white; width: 120px; margin-left: auto; margin-right: auto; margin-top: -10px; margin-bottom: 10px; font-size: 15px; border-radius: 3px;">Team Black</h3>');
  }
  else {
      $("#my_color").html('<h3 id="my_color" style="background-color: #87a5a8; border: 1px solid white;">Error: I don\'t know what color I am</h3>');
  }

  if (payload.game.whose_turn === 'white') {
      $("#my_color").append('<h4 style="font-style: italic; margin-top: 30px; margin-bottom: -35px;">It is white\'s turn! </h4>');
  }
  else if (payload.game.whose_turn === 'black') {
      $("#my_color").append('<h4 style="font-style: italic; margin-top: 30px; margin-bottom: -35px;">It is black\'s turn! </h4>');
  }
  else {
      $("#my_color").append('<h4 style="font-style: italic;">Error: Do not know whose turn it is</h4>');
  }
...
socket.on('game_over', (payload) => {
  ...
  let nodeB = $("<h1 style='margin-top: 50px';>Game Over &#127918;</h1>");
  let nodeC = $("<h2 style='margin: 10px;'>" + payload.who_won + " won!</h2>");
  let nodeD = $("<a href='lobby.html?username=" + username + "' class='btn btn-lg btn-success' role='button' style='margin: 40px;'>Return to Lobby</a>");
  nodeA.append(nodeB);
  nodeA.append(nodeC);
  nodeA.append(nodeD);
  nodeA.hide();
