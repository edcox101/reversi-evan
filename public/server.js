...
io.on('connection', (socket) => {
    ...
    /* Output a log messages on the server and send it to the clients */
 ...
 or
    ...
    socket.on('join_room', (payload) => {
        serverLog('Server receive a command', '\'join_room\'', JSON.stringify(payload));
        ...
        io.in(room).fetchSockets().then((sockets) => {
            ...
            /* Socket did join room */
                ...
                for (const member of sockets) {
                    ...
                    /* Tell every on that a new user has joined the chat room */
    ...
    socket.on('invite', (payload) => {
        serverLog('Server receive a command', '\'invite\'', JSON.stringify(payload));
        /* Check that the data coming from the client is good*/
        if ((typeof payload == 'undefined') || (payload === null)) {
            response = {};
            response.result = 'fail';
    ...
    socket.on('uninvite', (payload) => {
        serverLog('Server receive a command', '\'uninvite\'', JSON.stringify(payload));
        /* Check that the data coming from the client is good*/
        if ((typeof payload == 'undefined') || (payload === null)) {
            response = {};
            response.result = 'fail';
    ...
    socket.on('game_start', (payload) => {
        serverLog('Server receive a command', '\'game_start\'', JSON.stringify(payload));
        /* Check that the data coming from the client is good*/
        if ((typeof payload == 'undefined') || (payload === null)) {
            response = {};
            response.result = 'fail';
 ...
 or
    ...
    socket.on('play_token', (payload) => {
        ...
        if ((typeof username == 'undefined') || (username === null)) {
            let response = {};
            response.result = 'fail';
            response.message = 'play_token command did not come from a resgistered username';
            socket.emit('play_token_response', response);
            serverLog('play_token command failed', JSON.stringify(response));
            return;
        }
        ...
        if (
            ...
            let response = {
                result: 'fail',
                message: 'play_token played the right color, but bythe wrong player'
            }
            socket.emit('play_token_response', response);
            serverLog('play_token command failed', JSON.stringify(response));
            return;
